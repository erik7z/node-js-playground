...
const queueName = "journey_poc_bonus_status_change";
...
async function start() {
    const connection = await amqplib.connect('amqp://octopus_mq_user:uERdAWFCgExuuW7R@de2rmq03t.ctst.favorit:5672/master');
    const channel = await connection.createChannel();
    await channel.assertQueue(queueName, {durable: true});

    for await (const _ of Array(5_000)) {
